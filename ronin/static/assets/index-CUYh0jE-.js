const W={all:{label:"All Trials",kanji:"全",color:"#F5F2EB"},web:{label:"Web",kanji:"蜘蛛",color:"#E63946"},crypto:{label:"Crypto",kanji:"暗号",color:"#D4AF37"},cryptography:{label:"Crypto",kanji:"暗号",color:"#D4AF37"},pwn:{label:"Pwn",kanji:"刀切",color:"#8B5CF6"},binary:{label:"Binary",kanji:"刀切",color:"#8B5CF6"},forensics:{label:"Forensics",kanji:"影",color:"#06B6D4"},reversing:{label:"Reversing",kanji:"解体",color:"#10B981"},rev:{label:"Reversing",kanji:"解体",color:"#10B981"},misc:{label:"Misc",kanji:"雑",color:"#F97316"},miscellaneous:{label:"Misc",kanji:"雑",color:"#F97316"},osint:{label:"OSINT",kanji:"諜",color:"#EC4899"},hardware:{label:"Hardware",kanji:"機",color:"#EAB308"},warmup:{label:"Warmup",kanji:"初",color:"#14B8A6"},mobile:{label:"Mobile",kanji:"携",color:"#6366F1"},cloud:{label:"Cloud",kanji:"雲",color:"#38BDF8"},blockchain:{label:"Blockchain",kanji:"鎖",color:"#A855F7"},ai:{label:"AI",kanji:"知",color:"#06B6D4"}},q=[{color:"#F97316",kanji:"雑"},{color:"#EC4899",kanji:"諜"},{color:"#14B8A6",kanji:"初"},{color:"#EAB308",kanji:"機"},{color:"#6366F1",kanji:"携"},{color:"#A855F7",kanji:"鎖"},{color:"#06B6D4",kanji:"知"},{color:"#F43F5E",kanji:"斬"},{color:"#84CC16",kanji:"陣"}],z=[],et=[{rank:2,glyph:"銀",seal:"Silver Blade Seal",border:"border-slate-400/40",text:"text-slate-300",pad:"lg:pt-14"},{rank:1,glyph:"冠",seal:"Dragon Seal",border:"border-[#D4AF37]/60",text:"text-[#D4AF37]",pad:""},{rank:3,glyph:"銅",seal:"Bronze Torii Seal",border:"border-amber-700/50",text:"text-amber-600",pad:"lg:pt-20"}];function c(e){return document.querySelector(e)}function B(e){return document.querySelectorAll(e)}function O(e){if(!e)return{id:"unknown",label:"Trial",kanji:"試",color:"#F5F2EB"};const t=String(e).toLowerCase().replace(/\s+/g,"");if(W[t])return{id:t,...W[t]};const a=z.find(i=>i.id===t);if(a)return a;let n=0;for(let i=0;i<t.length;i++)n=(n<<5)-n+t.charCodeAt(i);const o=q[Math.abs(n)%q.length],s={id:t,label:String(e),kanji:o.kanji,color:o.color};return z.push(s),s}function V(e,t){let a;return(...n)=>{clearTimeout(a),a=setTimeout(()=>e(...n),t)}}function F(e){return String(e).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function nt(e){if(!e)return"";try{const a=new URL(e,window.location.origin).pathname.split("/").filter(Boolean).pop()||"";return decodeURIComponent(a)}catch{const a=String(e).split("?")[0].split("#")[0];return a.split("/").filter(Boolean).pop()||a}}function I(){const e=new IntersectionObserver(t=>{t.forEach(a=>{a.isIntersecting&&(a.target.classList.add("reveal-visible"),a.target.classList.remove("reveal-hidden"),e.unobserve(a.target))})},{rootMargin:"-40px"});B(".reveal").forEach(t=>{t.classList.add("reveal-hidden"),e.observe(t)})}function at(){setTimeout(()=>{B(".masked-line").forEach(e=>e.classList.add("masked-line-visible"))},60)}function ot(){setTimeout(()=>{B(".fade-in-anim").forEach(e=>{e.style.opacity="1",e.style.transform="none"})},60)}function st(){const e=c("#splash-intro");if(!e)return;const t=c("#splash-progress"),a=c("#splash-percent"),n=c("#splash-enter");let o=0;const s=setInterval(()=>{o+=2,t&&(t.style.width=o+"%"),a&&(a.textContent=o),o>=100&&(clearInterval(s),i())},60);function i(){e.classList.add("slashed"),setTimeout(()=>{e.style.display="none",document.body.classList.add("entered"),sessionStorage.setItem("ronin-entered","true"),P()},700)}n&&n.addEventListener("click",i)}function rt(){const e=c("#enso-canvas");if(!e)return;const t=e.getContext("2d"),a=Math.min(window.devicePixelRatio||1,1.5);let n=0,o=0;function s(){const b=e.parentElement;b&&(n=b.offsetWidth,o=b.offsetHeight,e.width=n*a,e.height=o*a,t.setTransform(a,0,0,a,0,0))}s(),window.addEventListener("resize",V(s,100));const i=Array.from({length:45},()=>({x:Math.random()*(n||1600),y:Math.random()*(o||900),r:Math.random()*1.5+.5,vx:(Math.random()-.5)*.2,vy:-(Math.random()*.3+.08),o:Math.random()*.45+.1,red:Math.random()<.35}));let l=0,r=0,d=0;function p(){const b=e.parentElement;if(!b)return;const v=c("#hero-logo");if(v&&v.offsetWidth>0){const y=v.getBoundingClientRect(),$=b.getBoundingClientRect();l=y.left-$.left+y.width/2,r=y.top-$.top+y.height/2,d=Math.max(y.width,y.height)*.46}else l=n>1024?n*.72:n*.5,r=o*.48,d=Math.min(n,o)*.28}let u=0;function x(b){u++,(u%20===0||l===0)&&p();const v=b;t.clearRect(0,0,n,o);const y=v*14e-5,$=Math.PI*.22,g=$/2+y,w=Math.PI*2-$/2+y,m=60;for(let f=0;f<m;f++){const h=f/m,A=g+(w-g)*h,tt=g+(w-g)*((f+1)/m),U=Math.sin(h*Math.PI);t.beginPath(),t.strokeStyle=`rgba(230,57,70,${.08+U*.12})`,t.lineWidth=1.5+U*10,t.arc(l,r,d,A,tt),t.stroke()}for(const f of i)f.x+=f.vx,f.y+=f.vy,f.y<-10&&(f.y=o+10,f.x=Math.random()*n),t.beginPath(),t.fillStyle=f.red?`rgba(230,57,70,${f.o})`:`rgba(245,242,235,${f.o*.7})`,t.arc(f.x,f.y,f.r,0,Math.PI*2),t.fill();requestAnimationFrame(x)}requestAnimationFrame(x)}function k(e,t="success"){let a=c(".toast-container");a||(a=document.createElement("div"),a.className="toast-container fixed bottom-6 right-6 z-[100] flex flex-col gap-2 pointer-events-none",document.body.appendChild(a));const n=document.createElement("div");n.className="modal-in px-4 py-3 font-mono2 text-xs max-w-sm shadow-2xl border pointer-events-auto flex items-start gap-3 backdrop-blur-md",t==="success"?(n.style.borderColor="rgba(16,185,129,0.5)",n.style.color="#6EE7B7",n.style.background="rgba(18,18,21,0.95)"):t==="info"||t==="hint"?(n.style.borderColor="rgba(212,175,55,0.6)",n.style.color="#F5D77F",n.style.background="rgba(18,18,21,0.95)"):(n.style.borderColor="rgba(230,57,70,0.6)",n.style.color="#FF4D5E",n.style.background="rgba(18,18,21,0.95)");const o=t==="success"?"達":t==="info"||t==="hint"?"灯":"警";n.innerHTML=`<span class="font-kanji text-base flex-shrink-0">${o}</span><span class="flex-1 leading-relaxed">${F(e)}</span>`,a.appendChild(n),setTimeout(()=>{n.style.opacity="0",n.style.transform="translateY(10px)",n.style.transition="all 0.3s ease",setTimeout(()=>n.remove(),300)},4e3)}let E=[],N=[],T="all",C="all";function R(){const e=window.location.hash;if(!e||e.length<=1)return;const t=e.match(/(?:.*-)?(\d+)$/);if(t){const a=parseInt(t[1],10);a&&E.some(n=>n.id===a)&&G(a)}}function it(){window.removeEventListener("hashchange",R),window.addEventListener("hashchange",R)}async function Y(){try{const t=await(await fetch("/api/v1/challenges")).json();t.success&&(E=t.data.map(a=>({id:a.id,title:a.name,category:a.category?a.category.toLowerCase().replace(/\s+/g,""):"misc",categoryLabel:a.category||"Misc",points:a.value,solves:a.solves,solved_by_me:a.solved_by_me,description:"",tags:a.tags||[]})),lt(),H(),it(),R())}catch(e){console.error("Failed to load challenges:",e),k("Failed to load trials","error")}}function lt(){const e=c("#category-tabs");if(!e)return;const t=[{id:"all",label:"All Trials",kanji:"全",color:"#71717A"}],a=new Set(["all"]);E.forEach(n=>{if(!a.has(n.category)){a.add(n.category);const o=O(n.category)||{id:n.category,label:n.categoryLabel,kanji:"試",color:"#F5F2EB"};t.push({id:n.category,label:n.categoryLabel||o.label,kanji:o.kanji,color:o.color})}}),e.innerHTML=t.map(n=>{const o=T===n.id;return`
      <button onclick="setCategory('${n.id}')" data-cat="${n.id}" class="challenge-cat-btn relative px-4 sm:px-5 py-3 flex items-center gap-2 ${o?"active":""}">
        ...
        <span class="font-mono2 text-[11px] tracking-[0.2em] uppercase transition-colors ${o?"text-[#F5F2EB]":"text-[#71717A] hover:text-[#A1A1AA]"}">${F(n.label)}</span>
  ...
  `}async function G(e){const t=E.find(i=>i.id===e);if(!t)return;try{const l=await(await fetch(`/api/v1/challenges/${e}`)).json();l.success&&(t.description=l.data.description,t.hints=(l.data.hints||[]).map(r=>typeof r=="object"&&r!==null?r:{id:r,cost:0}),t.files=l.data.files||[],t.connection_info=l.data.connection_info,t.attribution=l.data.attribution,t.tags=l.data.tags||[])}catch(i){console.error("Failed to load challenge details:",i)}const a=O(t.category)||{color:"#F5F2EB",label:t.categoryLabel,kanji:"試"},n=t.solved_by_me||N.includes(t.id),o=(t.title||"trial").toLowerCase().replace(/[^a-z0-9_-]/g,"_");window.location.hash!==`#${o}-${t.id}`&&history.replaceState(null,null,`#${o}-${t.id}`);const s=document.createElement("div");s.id="challenge-modal",s.className="fixed inset-0 z-[90] flex items-center justify-center p-4 fade-in",s.style.background="rgba(5,5,7,0.85)",s.style.backdropFilter="blur(8px)",s.innerHTML=`
    <div class="modal-in relative bg-[#121215] border border-[rgba(230,57,70,0.4)] text-[#F5F2EB] w-full max-w-2xl overflow-hidden max-h-[90vh] overflow-y-auto modal-scroll p-6 sm:p-8 shadow-2xl" onclick="event.stopPropagation()">
      ...
      <h2 class="font-heading text-2xl font-bold text-[#F5F2EB] tracking-wide">${F(t.title)}</h2>
      ...
      <div id="modal-panel-details">
        ...
        ${t.connection_info?`
          <div class="mb-6 mt-4">
            ...
            <div class="font-mono2 text-xs text-[#06B6D4] bg-[#06B6D4]/5 border border-[rgba(6,182,212,0.3)] px-4 py-3 break-all leading-relaxed">${F(t.connection_info).replace(/(https?:\/\/[^\s<]+)/g,'<a href="$1" target="_blank" rel="noopener" class="text-[#06B6D4] hover:text-[#22D3EE] underline underline-offset-4 decoration-[#06B6D4]/40 hover:decoration-[#22D3EE] transition-colors">$1</a>')}</div>
          </div>
        `:""}
          ...
          <div class="flex flex-wrap items-center gap-x-6 gap-y-2 border-y border-white/5 py-3 my-4">
            ...
            <div class="flex items-center gap-2">
              ...
              <div>
                ...
                <p class="font-heading text-sm font-bold text-[#D4AF37] tracking-wide">${F(r)}</p>
              </div>
            </div>
            `:""}
            ${l.length>0?`
            <div class="flex items-center gap-2 flex-wrap">
              ${l.map(d=>{const p=d.slice(5);return`<a href="${p}" target="_blank" rel="noopener" class="font-mono2 text-[11px] text-[#E63946] hover:text-[#FF4D5E] border border-[rgba(230,57,70,0.3)] hover:border-[rgba(230,57,70,0.6)] px-3 py-1.5 transition-colors">
                  <span class="font-kanji mr-1">鏈</span>${F(p.replace(/^https?:\/\//,"").split("/")[0])}
                </a>`}).join("")}
            </div>
            `:""}
          </div>
        ...
        ${t.files.length>0?`
          <div class="mb-6">
            ...
            <div class="flex flex-wrap gap-2">
              ${t.files.map(i=>{const l=nt(i);return`
                <a href="${i}" target="_blank" download="${F(l)}" title="${F(l)}" class="font-mono2 text-xs text-[#E63946] hover:text-[#FF4D5E] border border-[rgba(230,57,70,0.3)] hover:border-[rgba(230,57,70,0.6)] px-3 py-1.5 transition-colors flex items-center gap-1.5 max-w-full">
                  <span class="font-kanji mr-1 flex-shrink-0">文</span>
                  <span class="truncate">${F(l)}</span>
                </a>
              `}).join("")}
            </div>
          </div>
  ...
  `,s.addEventListener("click",_),document.body.appendChild(s),document.body.style.overflow="hidden"}async function ft(e,t){const a=c("#modal-tab-details-btn"),n=c("#modal-tab-solves-btn"),o=c("#modal-panel-details"),s=c("#modal-panel-solves");if(!(!a||!n||!o||!s))if(e==="details")a.className="font-heading text-xs font-bold uppercase tracking-widest text-[#E63946] border-b-2 border-[#E63946] pb-2.5 flex items-center gap-2 transition-all",n.className="font-heading text-xs font-bold uppercase tracking-widest text-[#71717A] hover:text-[#A1A1AA] pb-2.5 flex items-center gap-2 transition-all",o.classList.remove("hidden"),s.classList.add("hidden");else{n.className="font-heading text-xs font-bold uppercase tracking-widest text-[#E63946] border-b-2 border-[#E63946] pb-2.5 flex items-center gap-2 transition-all",a.className="font-heading text-xs font-bold uppercase tracking-widest text-[#71717A] hover:text-[#A1A1AA] pb-2.5 flex items-center gap-2 transition-all",o.classList.add("hidden"),s.classList.remove("hidden"),s.innerHTML='<div class="py-8 text-center font-mono2 text-xs text-[#71717A]">Unrolling the shadow ledger...</div>';try{const l=await(await fetch(`/api/v1/challenges/${t}/solves`)).json();if(l.success&&Array.isArray(l.data)){if(l.data.length===0){s.innerHTML=`
              ...
              <tbody class="font-mono2 text-xs divide-y divide-white/5">
                ${l.data.map((r,d)=>{const p=d===0,u=r.account_url||`/users/${r.account_id}`;return`
                    <tr class="hover:bg-white/[0.02] transition-colors ${p?"bg-[#E63946]/5":""}">
                      ...
                      <td class="py-3 px-3">
                        <a href="${u}" class="font-heading text-xs font-bold text-[#F5F2EB] hover:text-[#E63946] transition-colors">${F(r.name)}</a>
                      </td>
        ...
        `}}catch{s.innerHTML='<div class="py-8 text-center font-mono2 text-xs text-[#EF4444]">Failed to load victories ledger.</div>'}}}function _(){const e=c("#challenge-modal");e&&(e.remove(),document.body.style.overflow="",window.location.hash&&history.replaceState(null,null,window.location.pathname+window.location.search))}async function ut(e,t,a,n){var r,d;const o=E.find(p=>p.id===e),s=c(`#hint-wrapper-${t}`);if(!confirm(`Spend ${a} honor points to unlock Hint #${n}? This deduction cannot be undone.`))return;const l=((r=document.querySelector('meta[name="csrf-token"]'))==null?void 0:r.content)||((d=window.init)==null?void 0:d.csrfNonce)||"";try{const u=await(await fetch("/api/v1/unlocks",{method:"POST",headers:{"Content-Type":"application/json",Accept:"application/json","CSRF-Token":l},credentials:"same-origin",body:JSON.stringify({target:t,type:"hints"})})).json();if(u.success){const b=await(await fetch(`/api/v1/hints/${t}`)).json();if(b.success&&b.data&&(b.data.html||b.data.content)){const v=b.data.html||b.data.content;if(o&&o.hints){const y=o.hints.find($=>$.id===t);y&&(y.content=v)}s&&(s.outerHTML=`
            <div class="fade-in font-mono2 text-xs text-[#D4AF37]/90 border border-[rgba(212,175,55,0.25)] bg-[#D4AF37]/5 px-4 py-3 leading-relaxed">
              ...
              <div class="text-[#F5F2EB]/90 leading-relaxed">${M(v)}</div>
            </div>
          `),k(`Hint #${n} unlocked (-${a} PTS)`,"success")}else k("Hint unlocked! Please reopen the trial.","success")}else{const x=u.errors?typeof u.errors=="string"?u.errors:Object.values(u.errors).flat().join(" "):"Failed to unlock hint";if(x.toLowerCase().includes("already unlocked")){const v=await(await fetch(`/api/v1/hints/${t}`)).json();if(v.success&&v.data&&(v.data.html||v.data.content)){const y=v.data.html||v.data.content;if(o&&o.hints){const $=o.hints.find(g=>g.id===t);$&&($.content=y)}s&&(s.outerHTML=`
              <div class="fade-in font-mono2 text-xs text-[#D4AF37]/90 border border-[rgba(212,175,55,0.25)] bg-[#D4AF37]/5 px-4 py-3 leading-relaxed">
                <div class="flex items-center gap-2 mb-1.5 text-[10px] tracking-widest text-[#D4AF37] uppercase font-bold">
            ...
            `),k(`Hint #${n} was already unlocked by your clan!`,"success");return}}k(x||"Failed to unlock hint","error")}}catch(p){console.error("Unlock hint error:",p),k("Network error unlocking hint","error")}}async function ht(e,t,a){var i,l;const n=E.find(r=>r.id===e),o=c(`#hint-wrapper-${t}`),s=((i=document.querySelector('meta[name="csrf-token"]'))==null?void 0:i.content)||((l=window.init)==null?void 0:l.csrfNonce)||"";try{await fetch("/api/v1/unlocks",{method:"POST",headers:{"Content-Type":"application/json",Accept:"application/json","CSRF-Token":s},credentials:"same-origin",body:JSON.stringify({target:t,type:"hints"})});const d=await(await fetch(`/api/v1/hints/${t}`)).json();if(d.success&&d.data&&(d.data.html||d.data.content)){const p=d.data.html||d.data.content;if(n&&n.hints){const u=n.hints.find(x=>x.id===t);u&&(u.content=p)}o&&(o.outerHTML=`
          <div class="fade-in font-mono2 text-xs text-[#D4AF37]/90 border border-[rgba(212,175,55,0.25)] bg-[#D4AF37]/5 px-4 py-3 leading-relaxed">
      ...
      `}).join("")}else t.innerHTML="";if(a){const s=c("#col-victories");s&&(s.textContent=L==="teams"?"Warriors":"Victories");const i=(n=window.init)==null?void 0:n.userId,l=(o=window.init)==null?void 0:o.teamId;a.innerHTML=e.map(r=>{const d=r.name?r.name.charAt(0):"浪",p=L==="teams"?l!=null&&r.account_id===l:i!=null&&r.account_id===i,{rank:u,pct:x}=bt(r.score,X,r.pos),b=r.account_url?`<a href="${r.account_url}" class="font-heading text-sm font-bold tracking-wide text-[#F5F2EB] hover:text-[#E63946] transition-colors">${F(r.name)}</a>`:`<span class="font-heading text-sm font-bold tracking-wide text-[#F5F2EB]">${F(r.name)}</span>`;return`
        <tr class="border-b border-white/5 transition-colors hover:bg-[#1A1A1E] ${p?"bg-[#E63946]/5 border-l-2 border-l-[#E63946]":""}">
          ...
          <td class="px-6 py-4">
            <span class="inline-flex items-center gap-1.5 border px-2.5 py-1" style="border-color: ${u.border}; background: ${u.glow}" title="${u.title} — ${u.sub} (${x.toFixed(1)}% of total honor)">
              ...
              <span class="font-mono2 text-[9px] text-[#71717A]">${x.toFixed(0)}%</span>
            </span>
          </td>
      ...
      `}).join("")}I()}}function yt(e){const t=CATEGORIES.find(a=>a.id===String(e).toLowerCase().replace(/\s+/g,""));return t?t.color:"#94A3B8"}async function wt(){const e=window.location.pathname.match(/^\/users\/(\d+)/),t=e?e[1]:"me";try{const[a,n,o,s]=await Promise.all([fetch(`/api/v1/users/${t}`),fetch(`/api/v1/users/${t}/solves`),fetch(`/api/v1/users/${t}/fails`),fetch(`/api/v1/users/${t}/awards`)]),i=await a.json(),l=await n.json(),r=await o.json(),d=await s.json();i.success&&l.success&&kt(i.data,l.data||[],{fails:r.success&&r.data||[],awards:d.success&&d.data||[]})}catch(a){console.error("Failed to load profile:",a)}}function kt(e,t,a={}){const n=a.fails||[],o=a.awards||[],s=c("#stat-score"),i=c("#stat-solves"),l=c("#stat-fails");s&&(s.textContent=(e.score||0).toLocaleString()),i&&(i.textContent=t.length),l&&(l.textContent=n.length);const r=t.length+n.length,d=r>0?(t.length/r*100).toFixed(1):null,p=r>0?(n.length/r*100).toFixed(1):null,u=c("#solve-bar"),x=c("#fail-bar"),b=c("#solve-pct"),v=c("#fail-pct");if(u&&x){const g=d===null?50:parseFloat(d),w=p===null?50:parseFloat(p);u.style.width=g+"%",x.style.width=w+"%"}b&&(b.textContent=d===null?"—":d),v&&(v.textContent=p===null?"—":p);const y=c("#category-bar"),$=c("#category-legend");if(y&&$&&t.length>0){const g={};t.forEach(m=>{var h;const f=((h=m.challenge)==null?void 0:h.category)||"Unknown";g[f]=(g[f]||0)+1});const w=Object.keys(g).map(m=>({name:m,count:g[m],percent:g[m]/t.length*100,color:yt(m)}));y.innerHTML=w.map(m=>`<div class="h-full" style="width: ${m.percent}%; background-color: ${m.color}"></div>`).join(""),$.innerHTML=w.map(m=>`
      <div class="flex items-center gap-2">
        <span class="h-2.5 w-2.5 rounded-full" style="background-color: ${m.color}"></span>
        <span class="font-mono2 text-[10px] tracking-widest uppercase text-[#A1A1AA]">${F(m.name)} (${m.percent.toFixed(1)}%)</span>
      </div>
    `).join("")}if(window.echarts&&c("#user-score-graph")){const g=c("#user-score-graph"),w=[];t.forEach(h=>{var A;((A=h.challenge)==null?void 0:A.value)>0&&w.push({date:new Date(h.date).getTime(),value:h.challenge.value})}),o.forEach(h=>{h.value>0&&w.push({date:new Date(h.date).getTime(),value:h.value})}),w.sort((h,A)=>h.date-A.date);let m=0;const f=w.map(h=>(m+=h.value,[h.date,m]));if(f.length>0){g._echarts&&g._echarts.dispose();const h=window.echarts.init(g);g._echarts=h,h.setOption({backgroundColor:"transparent",grid:{top:10,right:10,bottom:24,left:46},tooltip:{trigger:"axis",backgroundColor:"#121215",borderColor:"rgba(230,57,70,0.4)",textStyle:{color:"#F5F2EB",fontFamily:"JetBrains Mono",fontSize:11}},xAxis:{type:"time",axisLine:{lineStyle:{color:"rgba(255,255,255,0.1)"}},axisLabel:{color:"#71717A",fontFamily:"JetBrains Mono",fontSize:10},splitLine:{show:!1}},yAxis:{type:"value",axisLine:{show:!1},axisLabel:{color:"#71717A",fontFamily:"JetBrains Mono",fontSize:10},splitLine:{lineStyle:{color:"rgba(255,255,255,0.05)"}}},series:[{type:"line",showSymbol:!0,symbolSize:5,smooth:!0,step:"end",lineStyle:{color:"#E63946",width:2},itemStyle:{color:"#E63946"},areaStyle:{color:"rgba(230,57,70,0.12)"},data:f}]})}}if(window.echarts&&c("#skills-radar")){const g=c("#skills-radar"),w={};t.forEach(f=>{var A;const h=((A=f.challenge)==null?void 0:A.category)||"Unknown";w[h]=(w[h]||0)+1});const m=Object.keys(w).map(f=>({skill:f,value:Math.min(w[f]*20+40,100)}));if(m.length>0){g._echarts&&g._echarts.dispose();const f=window.echarts.init(g);g._echarts=f,f.setOption({radar:{indicator:m.map(h=>({name:h.skill,max:100})),axisLine:{lineStyle:{color:"rgba(255,255,255,0.08)"}},splitLine:{lineStyle:{color:"rgba(255,255,255,0.08)"}},splitArea:{show:!1},axisName:{color:"#A1A1AA",fontFamily:"JetBrains Mono",fontSize:10}},series:[{type:"radar",data:[{value:m.map(h=>h.value),name:"Skills",areaStyle:{color:"rgba(230,57,70,0.22)"},lineStyle:{color:"#E63946",width:2},itemStyle:{color:"#E63946"}}]}]})}}}function Ft(){rt(),at(),ot(),$t()}async function $t(){try{const[e,t]=await Promise.all([fetch("/api/v1/challenges"),fetch("/api/v1/users")]),a=await e.json(),n=await t.json(),o=a.success?a.data.length:16,s=n.success?n.data.length:148,i=B(".landing-stat-value");i[0]&&(i[0].textContent=String(o).padStart(2,"0")),i[2]&&(i[2].textContent=String(s))}catch(e){console.error("Failed to load landing stats:",e)}}function At(){const e=c("#mobile-nav-toggle"),t=c("#mobile-nav-menu");e&&t&&e.addEventListener("click",()=>{t.classList.toggle("hidden"),t.classList.toggle("fade-in")})}let S=null;function Et(){if(!(typeof window.EventSource>"u"||S))try{S=new EventSource("/events"),S.addEventListener("notification",e=>{try{const t=JSON.parse(e.data),a=t.title||"Whisper",n=t.content?t.content.length>80?t.content.slice(0,80)+"...":t.content:"";k(`${a}: ${n}`,"info"),J()}catch{k("New whisper received","info"),J()}}),S.addEventListener("hint",()=>{k("A new hint has been unlocked for a trial","hint")}),S.addEventListener("scoreboard",()=>{window.location.pathname.startsWith("/scoreboard")&&K()}),S.onerror=()=>{}}catch(e){console.warn("Could not initiate /events listener:",e)}}function J(){const e=c("#notification-badge"),t=c("#mobile-notification-badge");[e,t].forEach(a=>{if(!a)return;const n=parseInt(a.textContent.trim(),10)||0;a.textContent=n+1,a.classList.remove("hidden")})}async function jt(){const e=c("#notification-badge"),t=c("#mobile-notification-badge");if(!(!e&&!t))try{const n=await(await fetch("/api/v1/notifications")).json();if(n.success&&Array.isArray(n.data)){const o=n.data.length;o>0&&[e,t].forEach(s=>{s&&(s.textContent=o,s.classList.remove("hidden"))})}}catch{}}function P(){const e=window.location.pathname;I(),At(),Et(),jt(),e==="/"||e===""?Ft():e==="/challenges"?(Y(),St()):e==="/scoreboard"?K():(e==="/user"||e.startsWith("/users/"))&&wt()}function St(){const e=c("#challenge-search");e&&e.addEventListener("input",V(()=>H(),300))}document.addEventListener("DOMContentLoaded",()=>{const e=c("#splash-intro"),t=sessionStorage.getItem("ronin-entered");e&&!t?st():(e&&(e.style.display="none",document.body.classList.add("entered")),P())});window.addEventListener("popstate",()=>{P()});window.setCategory=ct;window.setDifficulty=dt;window.switchBoard=xt;window.openChallengeModal=G;window.closeChallengeModal=_;window.switchChallengeModalTab=ft;window.submitFlag=gt;window.unlockHint=ut;window.revealFreeHint=ht;
